...
def test_volatility_features():
    ...
    expected_ratio_5_30 = expected_vol_5 / expected_vol_30

    expected_ratio_5_60 = expected_vol_5 / expected_vol_60
    ...
    expected_variance_5 = 0.01**2
    expected_variance_30 = 0.01**2
    expected_variance_60 = 0.01**2

    np.testing.assert_allclose(
        df["variance_5"].iloc[59],
        expected_variance_5,
    )

    np.testing.assert_allclose(
        df["variance_30"].iloc[59],
        expected_variance_30,
    )

    np.testing.assert_allclose(
        df["variance_60"].iloc[59],
        expected_variance_60,
    )

    np.testing.assert_allclose(df["variance_ratio_5_30"].iloc[59], 1.0)

    np.testing.assert_allclose(df["variance_ratio_5_60"].iloc[59], 1.0)
