...
# Only investigate the volatility regime that previously
# showed the strongest evidence.
...
def volatility_bucket(percentile):
...
# ============================================================
# PARAMETERIZED TRADE RESOLUTION
# ============================================================
...
def main():
    ...
    print("=" * 110)
    print("S2 PARAMETER SENSITIVITY — FIXED 40-60% REGIME")
    print("=" * 110)
    ...
    by_window_file = results_dir / "s2_parameter_sensitivity_40_60_by_window.csv"
    ...
    for (
        ...
        wins = int(active["wins"].sum())

        losses = int(active["losses"].sum())

        gross_profit = active["mean_R"]
        ...
        summary_records.append(
            {
                ...
                "wins": wins,
                "losses": losses,
                "overall_WR": (wins / total_trades if total_trades else np.nan),
                "total_R": active["total_R"].sum(),
                "mean_window_R": active["total_R"].mean(),
                "median_window_R": active["total_R"].median(),
                "mean_trade_R": (active["total_R"].sum() / total_trades),
    ...
    summary_file = results_dir / "s2_parameter_sensitivity_40_60_summary.csv"
    ...
    print()
    print("=" * 110)
    print("40-60% PARAMETER SENSITIVITY SUMMARY")
    print("=" * 110)
    ...
    if not summary.empty:
        ...
        print(ranked[columns].head(20).to_string(index=False))
    ...
    print()
    print("S2 40-60% PARAMETER SENSITIVITY COMPLETE")
