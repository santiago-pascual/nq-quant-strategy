...
KEY_FILES = [
    ...
    Path(
        "src/research/results/orb/"
        "orb_reconciliation_trades.csv"
    ),
    Path(
        "src/research/portfolio/"
        "22_full_system_funded_simulation.py"
    ),
    Path(
        "src/research/portfolio/"
        "22_mr_orb_portfolio_robustness.py"
    ),
    Path(
        "src/research/results/portfolio/funded/"
        "full_system_funded_combine_results.csv"
    ),
    Path(
        "src/research/results/portfolio/funded/"
        "full_system_funded_xfa_results.csv"
    ),
]
...
def load_trade_csv(path: Path) -> pd.DataFrame:
    ...
    if "entry_timestamp" not in df.columns:
        raise ValueError(
            f"{relpath(path)}: missing entry_timestamp"
        )

    df["entry_timestamp"] = normalize_timestamp_series(
        df["entry_timestamp"]
    )

    if df["entry_timestamp"].isna().any():
        raise ValueError(
            f"{relpath(path)}: invalid entry_timestamp values found"
        )
    ...
    if "r_multiple" not in df.columns:
        if "r" in df.columns:
            ...
            raise ValueError(
                f"{relpath(path)}: missing r_multiple/r/net_R"
            )
    ...
    if df["r_multiple"].isna().any():
        raise ValueError(
            f"{relpath(path)}: invalid R values found"
        )
...
def audit_final_oos(
    ...
    for strategy, path in OOS_SOURCES.items():
        ...
        oos_mask = (
            (df["entry_timestamp"] >= OFFICIAL_OOS_START)
            & (df["entry_timestamp"] <= OFFICIAL_OOS_END)
        )

        post_mask = (
            df["entry_timestamp"] >= POST_OOS_START
        )
        ...
        if actual == expected:
            findings.append(
                AuditFinding(
                    ...
                    category="OOS",
                    message=(
                        f"{strategy}: official OOS count = {actual}."
                    ),
                    path=relpath(path),
                )
            )
            ...
            if (
                ...
                findings.append(
                    AuditFinding(
                        ...
                        category="OOS",
                        message=(
                            f"{strategy}: OOS entries cross the frozen "
                            "OOS boundary."
                        ),
                        path=relpath(path),
                    )
    ...
    if total == EXPECTED_TOTAL_OOS:
        findings.append(
            AuditFinding(
                ...
                category="OOS",
                message=(
                    f"Combined official OOS count = {total}."
                ),
            )
        )
    else:
        findings.append(
            AuditFinding(
                ...
                category="OOS",
                message=(
                    f"Expected {EXPECTED_TOTAL_OOS} combined OOS "
                    f"trades, found {total}."
                ),
            )
        )
        ...
        if counts[strategy] == expected:
            findings.append(
                AuditFinding(
                    ...
                    category="OOS",
                    message=(
                        f"Combined OOS attribution: {strategy} = "
                        f"{counts[strategy]}."
                    ),
                )
            )
...
def calculate_frozen_metrics(
    ...
    daily = (
        combined.assign(
            date=combined["entry_timestamp"].dt.floor("D")
        )
...
# Seven manually inspected iloc[-1] findings.
...
SAFE_ILOC_LAST_PATTERNS = {
    "src/research/orb/13_orb_baseline.py": {
        465: (
            "SAFE: final RTH bar used for explicit RTH_CLOSE "
            "liquidation."
        ),
    },
    ...
    "src/research/orb/19_orb_funded_simulation.py": {
        2149: (
            "SAFE: reporting-only last historical trade timestamp."
        ),
    },
    ...
    "10_mr_portfolio_funded_simulation.py": {
        671: (
            "SAFE: reporting-only last historical event timestamp."
        ),
    },
    "src/research/mean_reversion/research/"
    "12_mr_3_strategy_visual_report.py": {
        262: (
            "SAFE: reporting-only end-of-dataset timestamp."
        ),
        703: (
...
def should_exclude_legacy(
    ...
    return any(
        token in lower
        for token in LEGACY_RESEARCH_EXCLUSIONS
    )
...
def audit_static_leakage(
    ...
    # -------------------------------------------------------------
    # Known future-target modules.
    #
    # These are deliberately retained as SAFE because their future
    # variables are targets for analysis/labeling, not final signal
    # inputs. They were manually inspected during the freeze audit.
    # -------------------------------------------------------------

    safe_target_files = {
        "src/targets.py": (
            "SAFE: future_vol_* and future_return_* are explicit "
            "future targets/labels, not final trading signals."
        ),
        "src/research/regime/hmm_transition_analysis.py": (
            "SAFE: next_state is descriptive transition analysis, "
            "not a final signal input."
        ),
    }

    for path_str, reason in safe_target_files.items():
        path = ROOT / path_str
    ...
    # -------------------------------------------------------------
    # Emit safe iloc classifications.
    # -------------------------------------------------------------
    ...
    for item in matches:
        findings.append(
            AuditFinding(
                ...
                category="LEAKAGE",
                message=(
                    f"{item['pattern']} at line {item['line']}: "
                    f"{item['explanation']}"
                ),
                path=item["path"],
                line=item["line"],
    ...
    print(
        f"Explicitly classified SAFE static cases: "
        f"{len(safe_matches)}"
    )
    ...
    if candidates:
        ...
        findings.append(
            AuditFinding(
                ...
                category="REPRO",
                message=(
                    "No research/validation entry points "
                    "were discovered."
                ),
            )
        )
...
def write_manifest(
    ...
    output = (
        ROOT
        / "src/research/results/portfolio/"
    ...
    print(
        f"Manifest written to: {relpath(output)}"
    )
...
def main() -> int:
    ...
    print(
        "Official OOS: "
        f"{OFFICIAL_OOS_START.date()} -> "
        f"{OFFICIAL_OOS_END.date()}"
    )
    print(
        f"Strategies: {', '.join(FINAL_STRATEGIES)}"
    )
    ...
    repro = audit_reproducibility_inventory(
        findings
    )
    ...
    if counts["FAIL"] > 0:
        ...
        print(
            "Resolve all FAIL findings before "
            "freezing the model."
        )
        print(
            f"Freeze manifest: {relpath(manifest)}"
        )
        return 1
    ...
    if counts["REVIEW"] > 0:
        print(
            "\nMODEL FREEZE GATE: "
            "PASS WITH MANUAL REVIEW ITEMS"
        )
        print(
            "No hard artifact/OOS/metric failure "
            "was detected."
        )
        print(
            "Remaining REVIEW findings require "
            "manual inspection."
        )
    else:
        print(
        ...
        print(
            "All artifact, OOS, frozen-metric, "
            "and static leakage checks passed."
        )
        print(
            "No unresolved leakage REVIEW items remain."
        )
